    ...
    public class AlcoholicContext : IDB<Alcoholic, int>
    {
        ...
        public Alcoholic Read(int key, bool noTracking = false, bool useNavigationProperties = false)
        {
            try
            {
                ...
                {
                    query = query.Include(i => i.DrinkingBuddies).Include(i => i.FavouriteDrinks).Include(i => i.Parties);
                }
        ...
        public void Update(Alcoholic item, bool useNavigationProperties = false)
        {
            try
            {
                ...
                if (useNavigationProperties)
                {
                    ...
                    {
                        Party PartyFromDB = _context.Parties.Find(party.ID);

                        if (PartyFromDB != null)
                        {
                            NewParties.Add(PartyFromDB);
                        }
                        else
                        {
                    ...
                    AlcoholicFromDB.Parties = NewParties;
                }
